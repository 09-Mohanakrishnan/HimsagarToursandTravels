...
import { TravelEvent, Blog } from "../../types";
...
export default function SEOManager() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const [configsRes, eventsRes, blogsRes, healthRes, kwRes] = await Promise.all([
        fetch("/api/admin/seo", { headers }),
        fetch("/api/events"),
        fetch("/api/admin/blogs?limit=200", { headers }),
      ...
      setBlogs((blogsData.blogs || blogsData || []).map((b: any) => ({ ...b, id: b._id || b.id })));
  ...
  const openEditor = (pageKey: string, defaultData: any = {}) => {
    ...
    const slug = (pageKey.startsWith("event::") || pageKey.startsWith("blog::")) ? pageKey.split("::")[1] : undefined;
    setFormData({ ...defaultData, ...existing, slug: existing.slug || slug });
    ...
    if (activeTab === "audit" || activeTab === "keywords" || activeTab === "actions") {
      if (pageKey.startsWith("event::") || pageKey.startsWith("blog::")) {
  ...
  const InputField = ({ label, field, type = "text", placeholder = "", rows = 1, readOnly = false }: any) => (
    <div className="space-y-2">
      ...
      {type === "textarea" ? (
        <textarea
          ...
          readOnly={readOnly}
          className={cn("w-full bg-slate-50 border border-gray-100 rounded-xl py-3 px-4 outline-none focus:border-brand-primary transition-all text-gray-700 text-sm font-medium resize-none", readOnly && "opacity-60 cursor-not-allowed bg-gray-100")}
        />
      ) : (
        <input
          type={type}
          ...
          placeholder={placeholder}
          readOnly={readOnly}
          className={cn("w-full bg-slate-50 border border-gray-100 rounded-xl py-3 px-4 outline-none focus:border-brand-primary transition-all text-gray-700 text-sm font-medium", readOnly && "opacity-60 cursor-not-allowed bg-gray-100")}
        />
      )}
    </div>
  );
  ...
  const getPreviewData = (key: string) => {
    ...
    if (key.startsWith("event::")) {
      ...
      if (blog) {
        defaultTitle = `${blog.title} | Himsagar Travels`;
        defaultDesc = blog.excerpt || defaultDesc;
        defaultImg = blog.featuredImage || defaultImg;
        defaultUrl = `https://himsagartravels.com/blog/${blog.slug}`;
      }
    } else {
  ...
  const allPreviewPages = [
    ...
    ...blogs.filter(b => b.status === "published").map(b => ({ key: `blog::${b.slug}`, label: b.title }))
  ];
      ...
      <AnimatePresence mode="wait">
        {editingKey && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="bg-white border border-brand-primary/30 rounded-3xl p-8 shadow-xl shadow-brand-primary/5 space-y-6 relative overflow-hidden">
            ...
            {(() => {
              const isDynamic = editingKey.startsWith("event::") || editingKey.startsWith("blog::");
              const dynamicType = editingKey.split("::")[0];
              const baseUrl = `https://himsagartravels.com/${dynamicType === "event" ? "tours" : "blog"}/`;
              const staticPage = !isDynamic ? staticPages.find(sp => sp.key === editingKey) : null;
              const staticUrl = staticPage ? `https://himsagartravels.com${staticPage.path}` : "";
              ...
              return (
                <form onSubmit={handleSave} className="space-y-6">
                  {isDynamic ? (
                    <div className="space-y-2 bg-slate-50 p-6 rounded-2xl border border-gray-100 mb-6">
                      <label className="text-[10px] uppercase tracking-widest font-black text-gray-400">Page URL (Slug)</label>
                      <input
                        type="text"
                        value={formData.slug || ""}
                        onChange={handleSlugChange}
                        className="w-full bg-white border border-gray-200 rounded-xl py-3 px-4 outline-none focus:border-brand-primary transition-all text-gray-700 text-sm font-medium"
                        placeholder="e.g. my-awesome-post"
                      />
                      <div className="text-[11px] text-emerald-600 font-mono bg-emerald-50 border border-emerald-100 p-2 rounded-lg mt-3 flex items-center gap-2">
                        <Globe size={12} />
                    ...
                    <div className="space-y-2 bg-slate-50 p-6 rounded-2xl border border-gray-100 mb-6">
                      <label className="text-[10px] uppercase tracking-widest font-black text-gray-400">Page URL</label>
                      <div className="text-[13px] text-gray-700 font-mono bg-white border border-gray-200 p-3 rounded-xl flex items-center gap-2 opacity-70 cursor-not-allowed">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <InputField label="H1 Tag" field="h1Tag" placeholder="Primary visible heading for this page" />
                    <InputField label="SEO Title" field="seoTitle" placeholder="Page Title | Himsagar Travels" />
                  </div>
                  <InputField label="Canonical URL" field="canonicalUrl" placeholder="https://himsagartravels.com/..." readOnly={isDynamic} />
            ...
            {blogs.filter(b => b.status === "published").length > 0 && (
              <>
                <div className="border-t border-gray-100 pt-6 mt-6">
                  <h3 className="text-lg font-serif text-gray-800 mb-6">Blog Post Pages</h3>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {blogs.filter(b => b.status === "published").map((blog) => {
                    ...
                    const conf = seoConfigs.find(c => c.pageKey === pageKey);
                    return (
                      <div key={blog.id} className="p-6 rounded-2xl border border-gray-100 bg-slate-50 hover:border-brand-primary/30 transition-all flex justify-between items-center group">
                        <div>
                          <h4 className="font-bold text-gray-800 line-clamp-1">{blog.title}</h4>
                          <p className="text-xs text-gray-400 mt-1 font-mono">/blog/{blog.slug}</p>
                          {conf ? (
                            <span className="inline-block mt-2 px-2 py-0.5 bg-emerald-100 text-emerald-600 text-[9px] font-black uppercase tracking-widest rounded">Customized</span>
                          ) : (
                            <span className="inline-block mt-2 px-2 py-0.5 bg-gray-200 text-gray-500 text-[9px] font-black uppercase tracking-widest rounded">Auto-Generated</span>
                        ...
                        <button 
                          onClick={() => openEditor(pageKey, { seoTitle: `${blog.title} | Himsagar Travels`, canonicalUrl: `https://himsagartravels.com/blog/${blog.slug}`, metaDescription: blog.excerpt?.substring(0, 160) })}
                          className="px-4 py-2 bg-white border border-gray-200 rounded-xl text-[10px] uppercase tracking-widest font-black text-brand-primary opacity-0 group-hover:opacity-100 transition-all hover:bg-brand-primary hover:text-white"
                        >
                          Edit
                        </button>
